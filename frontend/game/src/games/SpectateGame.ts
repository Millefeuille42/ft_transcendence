...
import P5 from "p5";
import Rod from "../classes/specGameClasses/Rod";
import Ball from "../classes/specGameClasses/Ball";
...
class SpectateGame implements IScreen {
	sleep: boolean = false
	...
	screenLoop(p5: P5): boolean {
		...
		if (!net.started) {
			...
			p5.text("Waiting for match to start", 0, p5.height / 2.5, p5.width)
			return false
		}
